...
def iterativeDeepening(problem):
    ...
    while (not generalReturn and iterativeLimit<20):
        iterativeLimit += 1
        generalReturn = General_Search(problem, SearchEnum.ITERATIVE_DEEPENING_SEARCH)
    iterativeLimit = 0
...
def General_Search(problem, search):
    ...
    # Pathing to the given node | Cost of the given node
    queue = Queue.Queue(
...
def expand_queue(queue, newNodesToAddToQueue, problem, search):
    ...
    if search == SearchEnum.DEPTH_FIRST_SEARCH:
        ...
        """
        queue.insertNodesAtFront([x for x in newNodesToAddToQueue if len(x.path) <= (lengthLimit)])
        ...
        if (iterativeLimit == 0):
            ...
            Include your solution for Depth-limited here. Hint: 1 line of code (a call to a function provided) sufficies, but you can have more lines if you need to.
            """
            # if len(queue.nodes[0]) > iterativeLimit:
            # iterativeLimit


            # if len(newNodesToAddToQueue) is not 0 and len(newNodesToAddToQueue[0].path) <= (iterativeLimit + 1):
            #     queue.insertNodesAtFront(newNodesToAddToQueue)

            queue.insertNodesAtFront([x for x in newNodesToAddToQueue if len(x.path) <= (iterativeLimit+1)])

            # # path = queue.Remove_Front()
            # if len(path.path) < iterativeLimit:
            #     queue.insertNodesAtFront(newNodesToAddToQueue)
            # else:
            #     queue.insertNodesAtFront(path)

            # if len(newNodesToAddToQueue[0].path) >= iterativeLimit:
            #     # newNodesToAddToQueue= Queue.Node([problem.getState(initialState)])
            #     queue.insertNodesAtFront(newNodesToAddToQueue)
